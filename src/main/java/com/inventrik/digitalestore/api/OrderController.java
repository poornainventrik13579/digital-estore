...
public class OrderController {
     ...
     *
     * Regular users can only see their own orders. Admins and Tenants can see all orders.
     */
    @GetMapping
    @PreAuthorize("hasAnyAuthority('ROLE_USER', 'ROLE_ADMIN', 'ROLE_TENANT')")
    @Operation(summary = "Get all orders (latest first). Regular users see only their orders.")
    public ResponseEntity<List<OrderResponse>> getAllOrders(
        ...
        boolean canAccessAllOrders = authentication.getAuthorities().stream()
                ...
                .anyMatch(authority -> authority.equals("ROLE_ADMIN") || authority.equals("ROLE_TENANT"));

        return ResponseEntity.ok(orderService.getAllOrders(tenantId, username, canAccessAllOrders, status));
    }
